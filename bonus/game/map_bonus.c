...
/*   Updated: 2025/01/08 17:13:43 by mskhairi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_player(t_data *data, t_coor coor, mlx_image_t *img)
{
	...
	while (i < 4)
	{
		...
		while (j < 4)
		{
			my_mlx_put_pixel(data, img, (t_coor){coor.x + i - 2, coor.y + j
				- 2}, get_rgba(96, 86, 120, 255));
			j++;
		}
		i++;
	}
	...
	while (j < MINI_HEIGHT)
	{
		...
		while (i < MINI_WIDTH)
		{
			if (x >= 0 && x < (data->map.width * TILE_SIZE) && y >= 0
				&& y < (data->map.heigth * TILE_SIZE))
			{
				if (data->map.grid[(int)(y / TILE_SIZE)][(int)(x
						/ TILE_SIZE)] == '1')
					my_mlx_put_pixel(data, data->ft_3d, (t_coor){i, j},
						get_rgba(0, 0, 0, 255));
				if (data->map.grid[(int)(y / TILE_SIZE)][(int)(x
						/ TILE_SIZE)] == '0' || ft_strchr("NSEW",
						data->map.grid[(int)(y / TILE_SIZE)][(int)(x
							/ TILE_SIZE)]))
					my_mlx_put_pixel(data, data->ft_3d, (t_coor){i, j},
